...
print(np.asarray((unique, counts)).T, len(labels))
...
for row in meal_dataset:
    ...
    f34 = np.array(fn_fft(row))
    f56 = np.array(fn_zero_crossings(row, len(row)))
    f7 = np.array(skew(row))
    f8 = np.array(np.mean(row))
    f9 = np.array(np.median(row))
    f_all = np.concatenate((f1, f2, f34, f56, f7, f8, f9), axis=None)
    if i == 0:
        meal_features = f_all
        i = 1
    else:
